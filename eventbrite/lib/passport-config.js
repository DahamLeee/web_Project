...
module.exports = function(passport){
  ...
  passport.use(new FacebookStrategy({
    clientID : '312659525878255',
    clientSecret : '32616985e32c4b42546e31d90c67bb13',
    callbackURL : 'http://localhost:3000/auth/facebook/callback',
    profileFields : ['email', 'name', 'picture']
  }, async(token, refreshToken, profile, done) => {
    console.log('Facebook', profile); //profile 정보
    try{
      ...
      // 같은 facebook id를 가진 사용자가 있는지 판별
      var user = await User.findOne({'facebook.id' : profile.id});
      if(!user){
        // 없다면, 혹시 같은 email 이라도 가진 사용자가 있나? local id와 facebook email이 같을경우
        if(email){
